...
내부 문서를 보호하는 민감도 레이블과 HR 직원 데이터 전용 하위 레이블을 만드는 데 성공했습니다.
...
1. 오른쪽의 **새 규칙** 플라이아웃 패널의 **조건** 섹션에 있는 **콘텐츠가 다음을 포함**에서 **추가** > **학습 가능한 분류자**를 선택하고 학습 가능한 **Paystub** 학습 가능 분류자를 다시 추가합니다.
   ...
   **이 작업을 다시 수행하는 이유는 무엇일까요?** 이 작업을 이전에 수행할 때 파일 및 이메일에 자동으로 레이블을 지정하는 조건을 설정했습니다. 이번에는 SharePoint 및 OneDrive에서 파일에 자동으로 레이블을 지정하는 추가 단계가 진행됩니다.
