...
1. **X**를 사용하여 오른쪽 창을 닫은 다음 Mod 관리자 계정에서 로그아웃합니다.
...
1. **무시할 구분 기호 및 문장 부호 선택 드롭다운을 선택**하고 *하이픈*, *마침표*, *띄어쓰기*, *여는 괄호*, *닫는 괄호*를 선택합니다.

1. 첫 번째 **스키마 필드 이름**에 *Name*을 입력하고 **필드 검색 가능**을 선택합니다.

1. 새 EDM 스키마 창 하단에서 **+ 스키마 데이터 필드 추가**를 선택합니다.

1. **스키마 필드 이름**의 **스키마 필드 2** 아래에 *Birthdate*를 입력합니다.

1. **+ 스키마 데이터 필드 추가**를 다시 선택합니다.

1. **스키마 필드 이름**의 **스키마 필드 3** 아래에 *StreetAddress*를 입력합니다.

1. 마지막으로 **+ 스키마 데이터 필드 추가**를 선택합니다.
