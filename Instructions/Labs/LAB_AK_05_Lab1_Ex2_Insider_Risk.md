...
1. **콘텐츠의 우선 순위를 지정할지 여부 결정 페이지에서** 중요한 정보 유형**만 **선택한 상태로 두고 다음**을 선택합니다**.
...
1. 이 정책을** 트리거할 활동 선택에서 **다음을 선택합니다.
   ...
   - **Microsoft 365 위치에서 제거된 위치 다운로드**
    ...
    >**참고**: 정책 트리거를 선택할 수 없는 경우 표시기를 켜는 팁이 있을 수 있습니다. 이 옵션을 사용할 수 있는 경우 표시기** 켜기를 선택합니다**. **표시기 선택에서 팝업을 켜**고 Office 표시**기에 대해 **모두** 선택 옆**의 검사 상자를 클릭한 다음 저장**을 선택합니다**.

1. **다음**을 선택합니다.

1. **이벤트** 트리거에 대한 임계값 선택 페이지에서 기본 제공 임계값 적용(권장)**을 선택한 **다음, 다음**을 선택합니다**.
...
1. **정책을 만든** 페이지에서 완료**를 선택합니다**.
    ...
    >**참고:** 이 페이지에서 설명한 대로 정책 일치가 경고 탭에 표시되기까지 최대 24시간이 걸릴 수 있습니다.
