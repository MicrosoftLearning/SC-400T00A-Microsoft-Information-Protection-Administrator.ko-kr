...
1. 이름 **필드의** **이름 및 우선 순위 사용자 그룹 설명** 페이지에서 **재무 팀을** 입력합니다. **설명** 텍스트 상자에 **재무 운영, 예산 및 보고를 관리하는 팀 구성원을 입력하고** **다음**을 선택합니다.
...
1. **정책 이름 지정 페이지의** **이름** 필드에 **재무 데이터 보호를** 입력하고 **설명** 필드에 **중요한 금융 데이터 액세스 모니터링을** 입력한 다음 **, 다음**을 선택합니다.
...
1. **중요한 정보 유형 추가 또는 편집** 창에서 *은행을* 검색하고 **미국 은행 계좌 번호** 및 **IBAN(International Banking Account Number) 옆에 있는** 검사 상자를 선택합니다. 그런 다음 *신용을* 검색하고 **신용 카드 번호** 옆에 있는 검사 상자를 선택한 다음 **, 추가**를 선택하여 3가지 중요한 정보 유형을 추가합니다.
...
이 작업에서는 Microsoft Purview의 참가자 위험 관리에서 알림 템플릿을 만듭니다. 이 템플릿을 사용하면 위험 활동에 대한 사례가 생성될 때 사용자에게 전자 메일 메시지를 자동으로 보내거나 미리 알림 역할을 하거나 규정 준수 교육을 위한 정보를 제공할 수 있습니다.
