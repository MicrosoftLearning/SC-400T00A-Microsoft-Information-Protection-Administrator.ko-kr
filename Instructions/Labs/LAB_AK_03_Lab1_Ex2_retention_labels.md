...
1. **보존 기간 이후에 수행되는 작업 선택 페이지에서 보존** **설정 비활성화를** 선택한 다음 **, 다음**을 선택합니다.
...
1. **기간 정의** 페이지 입력에서 다음을 수행합니다.
    ...
    - **항목 유지**: 드롭다운 목록을 선택하고 **사용자 지정**을 선택합니다. 연도에 3을 입력합니다.
    - 항목**이 만들어진 시기에 따라 보존 기간을 시작**합니다.
...
1. **보존 레이블이 만들어짐** 페이지에서 **아무 작업도 수행하지 않음**을 선택한 다음 **완료**를 선택합니다.

보존 기간이 7년인 VAT 환급 문서용 보존 레이블과 보존 기간이 3년인 신용 카드 영수증용 보존 레이블을 만들었습니다.

## 작업 2 - 보존 레이블 게시

이제 작업 1에서 게시된 레이블을 재무 사용자가 Exchange 전자 메일 및 SharePoint 문서의 문서에 적용할 수 있도록 VAT 반환 보존 레이블을 게시합니다.
...
1. 작업 1에서 만든 **VAT 반환 및 지원 문서 레이블을** 선택합니다.
...
1. **레이블을 게시할 위치 선택** 페이지에서 **특정 위치를 선택하고** 사용하도록 설정을 선택합니다.

    - **Exchange 사서함**
    - **SharePoint 클래식 및 통신 사이트**
    - **OneDrive 계정**
    - **Microsoft 365 그룹 사서함 & 사이트** 사용 안 함

1. **다음**을 선택합니다.

1. **정책 이름 입력**에서 다음을 입력합니다.
...
1. **이 레이블을 적용할 콘텐츠 유형 선택** 페이지에서 **중요한 정보가 포함된 콘텐츠에 레이블 적용을** 선택한 다음 **, 다음**을 선택합니다.
...
1. **정책을 적용할 위치 선택** 페이지에서 다음 옵션 **켜**기를 켭니다.
...
1. **정책 테스트 또는 실행 여부 결정**에서 **정책 켜기를** 선택한 다음 **, 다음**을 선택합니다.

1. **검토 후 완료** 페이지에서 **제출**을 선택합니다.  정책이 만들어지면 **완료**를 선택합니다.

1. 오른쪽 위 모서리에서 이미지를 선택하고 로그아웃을 선택하여 Joni의 계정 **에서 로그아웃**합니다.
...
1. **Microsoft Edge**에서 으로 이동하여 **https://outlook.office.com/** **Megan Bowen**의 계정으로 로그인합니다.
...
1. Megan의 받은 편지함에서 마우스 오른쪽 단추가 있는 첫 번째 전자 메일을 선택한 다음, **VAT 반환 및 지원 문서를 > 정책 할당 > 고급 작업을** 선택합니다. 새로 만든 레이블을 사용할 수 없는 경우 이 연습에서 **1개월 삭제** 를 선택합니다.
    ...
    >**참고**: 게시된 보존 레이블은 Exchange Online 표시되는 데 7일이 걸릴 수 있으며 사서함에는 10MB의 데이터가 포함되어야 합니다.

1. **Outlook** 창을 열린 상태로 유지합니다.
...
1. 여전히 클라이언트 1 VM(LON-CL1)에 **lon-cl1\admin** 계정으로 로그인해야 합니다. **Outlook**이 있는 Microsoft Edge 브라우저 창은 계속 열려 있어야 하며 **여전히 Megan Bowen**으로 로그인해야 합니다.

1. 왼쪽 창에서 **받은 편지함**을 선택하고 마우스 오른쪽 단추를 클릭합니다.

1. **새 하위 폴더 만들기를** 선택합니다.

1. 빈 필드가 있는 커서가 새 파일 이름의 엔더에 표시됩니다. 이 필드에 _VAT 반환을_ 입력한 다음 **저장**을 선택합니다.

1. 왼쪽 패널에서 새로 만든 **VAT Returns** 폴더를 마우스 오른쪽 단추로 클릭하고 **VAT 반환 및 지원 문서에 > 정책 할당을** 선택합니다. 새로 만든 레이블을 사용할 수 없는 경우 이 연습에서 **5년 삭제** 를 선택합니다.
...
1. Microsoft 365 방문 페이지의 왼쪽 위 모서리에 있는 앱 시작 관리자 아이콘(점이 9개인 아이콘)을 선택한 다음, 하위 메뉴에서 **SharePoint** 를 선택합니다.
1. 메시지가 표시되면 **SharePoint 시작 페이지** 창을 닫습니다.

1. SharePoint 방문 페이지에서 _통신 사이트를_ 검색한 다음 검색 결과에서 **통신 사이트를** 선택합니다.
...
1. 강조 표시된 문서의 경우 가로 타원 **...**, 단추를 선택한 다음 추가 **> 준수 세부 정보를** 선택합니다.

1. 문서의 **준수 세부 정보를** 표시하는 새 창이 열립니다. **레이블 상태에** 대해 **없음**을 선택하여 **레이블 적용** 창을 엽니다.

1. 옵션을 사용할 수 있는 경우 **레이블 적용** 을 **VAT 반환 및 지원 문서(7년 동안 보존)로** 설정한 다음 **, 저장**을 선택합니다. 보존 레이블이 게시되려면 시간이 다소 걸릴 수도 있으므로 이 옵션을 즉시 사용하지는 못할 수도 있습니다. 이 옵션을 사용할 수 없어도 다음 작업을 계속 진행하세요. 나중에 언제든 이 작업으로 돌아와서 다시 시도해 보세요.
...
1. Microsoft 365 방문 페이지의 왼쪽 위 모서리에서 앱 시작 관리자 아이콘(점이 9개인 아이콘)을 선택한 다음, 하위 메뉴에서 **OneDrive** 를 선택합니다.

1. 왼쪽 탐색 창에서 **내 파일을** 선택한 다음, **계약업체 법률 Info.docx**왼쪽에 있는 확인란을 선택합니다.

1. 강조 표시된 문서의 경우 가로 타 **원... 단추를** 선택한 다음 세부 **정보를** 선택합니다.

1. 오른쪽 플라이아웃 페이지의 **속성** 아래에 **레이블 적용** 옵션이 표시됩니다. **레이블 선택을 선택하여** 드롭다운을 확장한 다음, 사용 가능한 경우 **VAT 반환 및 지원 문서를** 선택합니다.
