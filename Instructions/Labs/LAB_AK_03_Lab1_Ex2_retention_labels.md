...
6. **보존 레이블의 이름 지정** 페이지의 **이름**, **사용자를 위한 설명** 및 **관리자를 위한 설명** 에 다음 정보를 각각 입력합니다.
...
11. **검토 후 완료** 페이지에서 **레이블 만들기** 단추를 선택합니다.  보존 레이블 생성됨 페이지에서 아무 작업도 하지 않음을 선택하고 **완료** 를 선택합니다.

12. **데이터 수명 주기 관리** 페이지의 **레이블** 탭으로 돌아갑니다.  이후 연습에서 레이블을 게시하겠습니다.
...
16. **보존 설정 정의** 페이지에서 **특정 기간 동안 항목 보존** 설정을 사용하도록 설정해야 합니다. 드롭다운 목록에서 **사용자 지정** 을 선택하고 다음 정보를 설정합니다.
    - **보존 기간**: 3년
    - **보존 기간 시작 기준**: 항목 작성 시
    - **보존 기간 종료 시**: 아무 작업도 하지 않음
...
18. **검토 후 완료** 페이지에서 **레이블 만들기** 단추를 선택합니다. 보존 레이블 생성됨 페이지에서 아무 작업도 하지 않음을 선택하고 **완료** 를 선택합니다.
...
4. **데이터 수명 주기 관리** 페이지에서 **레이블** 탭을 선택합니다.
...
8. **만들 보존 정책 유형 선택** 페이지에서 **정적** 항목을 선택합니다.
...
16. **만들 보존 정책 유형 선택** 페이지에서 **정적** 항목을 선택하고 **다음** 을 클릭합니다. 

17. **정책을 적용할 위치 선택** 페이지에서 **Exchange 전자 메일, OneDrive 계정, SharePoint 사이트 및 Microsoft 365 그룹** 옵션을 사용하도록 설정하고 **다음** 을 선택합니다.

18. **자동 적용할 레이블 선택** 페이지에서 **다음** 을 선택합니다.

19. **검토 및 완료** 페이지에서 **제출** 단추를 선택합니다.  정책이 만들어지면 **완료** 를 선택합니다.
...
2. 페이지 아래쪽의 작업 표시줄에서 시작 단추를 선택하고 아래쪽으로 스크롤한 다음 **Outlook** 을 선택합니다. 필요한 경우 **Megan Bowen**(MeganB@WWLxZZZZZZ.onmicrosoft.com)으로 로그인합니다(여기서 ZZZZZZ는 랩 호스팅 공급자가 제공한 고유 테넌트 ID임).  Megan의 암호는 랩 호스팅 공급자가 제공합니다.
...
6. **VAT 환급 및 지원 문서** 정책이 사용 가능하면 해당 정책을 설정하고, 아니면 기존 정책에서 **한 달 후 삭제** 를 선택합니다(이 연습 내에서 설정을 적용하기 위한 용도로만 사용됨).  **참고** 게시된 보존 레이블이 Exchange Online에 표시되려면 7일이 걸릴 수 있습니다. 사서함에는 데이터 10MB가 포함되어 있어야 합니다.
...
7.  **VAT 환급 및 지원 문서 정책** 이 사용 가능하면 **폴더 정책** 드롭다운 목록에서 해당 정책을 설정하고, 아니면 기존 정책에서 **5년 후 삭제** 를 선택합니다(이 연습 내에서 설정을 적용하기 위한 용도로만 사용됨). **참고** 게시된 보존 레이블이 Exchange Online에 표시되려면 7일이 걸릴 수 있습니다. 사서함에는 데이터 10MB가 포함되어 있어야 합니다.
...
4. SharePoint 시작 페이지에서 아래쪽으로 스크롤한 다음 **커뮤니케이션 사이트** SharePoint 사이트를 선택합니다. **참고**: 검색창에 **커뮤니케이션 사이트** 를 입력해 검색합니다.
...
11. **VAT 환급 및 지원 문서** 옵션이 사용 가능하면 **레이블 적용** 을 해당 옵션으로 설정하고 **저장** 을 선택합니다. 보존 레이블이 게시되려면 시간이 다소 걸릴 수도 있으므로 이 옵션을 즉시 사용하지는 못할 수도 있습니다. 이 옵션을 사용할 수 없어도 다음 작업을 계속 진행하세요. 나중에 언제든 이 작업으로 돌아와서 다시 시도해 보세요.
...
8. **VAT 환급 및 지원 문서** 옵션이 사용 가능하면 **보존 레이블 적용** 을 해당 옵션으로 설정합니다. 보존 레이블이 게시되려면 시간이 다소 걸릴 수도 있으므로 이 옵션을 즉시 사용하지는 못할 수도 있습니다. 이 옵션을 사용할 수 없어도 다음 작업을 계속 진행하세요. 나중에 언제든 이 작업으로 돌아와서 다시 시도해 보세요.
