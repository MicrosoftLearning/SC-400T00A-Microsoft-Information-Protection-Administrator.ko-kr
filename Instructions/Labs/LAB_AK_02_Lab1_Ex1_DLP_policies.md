...
3. **Microsoft Purview** 포털의 왼쪽 탐색 창에서 **정책** 을 선택하고 **데이터** 에서 **데이터 손실 방지** 를 선택합니다.

1. **데이터 손실 방지** 창에서 **정책** 탭을 선택하고 **+정책 만들기** 를 선택하여 새 데이터 손실 방지 정책 만들기 마법사를 시작합니다.
...
1. **DLP 정책 이름 지정** 페이지의 **이름** 필드에 신용 카드 DLP 정책을 입력하고 신용 카드 번호가 공유되지 않도록 보호합니다.  **설명** 필드에 신용 카드 번호를 공유할 수 없도록 보호하는 정책입니다.를 입력합니다. **다음** 을 선택합니다.
...
1. **규칙 만들기** 페이지의 **이름** 필드에 *신용 카드 정보* 를 입력합니다.
...
1. **규칙 만들기** 페이지에서 가 **+ 작업 추가** 를 선택하고 **Microsoft 365 위치에서 액세스 제한 또는 콘텐츠 암호화** 를 선택합니다.

1. **Microsoft 365 위치에서 액세스 제한** 또는 콘텐츠 암호화 앞에 있는 체크박스를 선택하고 **모든 사람 차단** 을 선택합니다.

1. **규칙 만들기** 페이지의 **사용자 알림** 섹션에서 스위치를 선택하여 **켜기** 위치로 이동합니다.

1. **규칙 만들기** 페이지에 있는 **사용자 재정의** 섹션의 **M365 서비스에서 재정의 허용** 에서 **M365 서비스에서 재정의를 허용합니다. Exchange, Sharepoint, OneDrive, Teams의 사용자가 정책 제한을 무시할 수 있습니다.** 상자를 선택합니다.

    참고: **M365 서비스에서 재정의 허용** 확인란을 선택할 수 없다면 이전 단계의 **사용자 알림 >> Microsoft 365 서비스** 섹션 아래의 **규칙 만들기** 페이지에서 찾을 수 있는 **정책 팁으로 Office 365 사용자에게 알림** 확인란을 선택합니다. 그런 다음, **M365 서비스에서 재정의를 허용합니다. Exchange, Sharepoint, OneDrive, Teams의 사용자가 정책 제한을 무시할 수 있습니다.** 상자를 선택합니다.

1. **재정의하려면 비즈니스 타당성 필요** 상자를 선택합니다.

1. **인시던트 보고서** 섹션의 **관리 경고 및 보고서에서 이 심각도 수준 사용** 드롭다운에서 **낮음** 을 선택합니다.

1. **저장**, **다음** 을 차례로 선택합니다.

1. **정책 테스트 또는 켜기** 페이지에서 **먼저 테스트** 를 선택하고 **테스트 모드에 있을 때 정책 팁 표시** 를 선택합니다.
...
1. **Purview 포털** 의 왼쪽 탐색 창에서 **정책** 을 선택하고 **데이터** 에서 **데이터 손실 방지** 를 선택합니다.

1. **데이터 손실 방지** 창에서 **정책** 탭을 선택하고 이름이 **신용 카드 DLP 정책** 인 정책을 선택한 후에 **정책 편집**(연필 아이콘)을 선택하여 정책 마법사를 엽니다.
...
1. **데이터 손실 방지** 창에서 **정책** 탭을 선택한 다음, **신용 카드 DLP 정책** 이라는 정책을 선택하고 **정책 편집**(연필)을 선택하여 정책 마법사를 엽니다.

1. **정책 테스트 또는 켜기** 페이지가 표시될 때까지 **다음** 을 선택하고 **즉시 켜기** 를 선택합니다.

1. **다음**, **제출** 을 차례로 선택하여 정책을 활성화합니다.
...
1. **데이터 손실 방지** 창에서 **정책** 탭을 선택하고 **EmployeeID DLP 정책** 옆에 있는 세로 점 3개를 선택하여 **작업** 선택 항목을 엽니다.
...
1. **Microsoft Edge** 를 열고 **https://portal.cloudappsecurity.com** 으로 이동한 다음, 클라우드용 Microsoft Defender 앱 포털에 **MOD 관리자** 로 로그인합니다. admin@WWLxZZZZZZ.onmicrosoft.com(여기서 ZZZZZZ는 랩 호스팅 공급자가 제공한 고유 테넌트 ID입니다).  관리자의 암호는 랩 호스팅 공급자가 제공합니다.

1. 오른쪽 위의 프로필 정보 옆에 있는 톱니 바퀴 모양 **설정** 아이콘을 선택하고 드롭다운 메뉴에서 **설정** 을 선택합니다.

1. **설정** 페이지의 **정보 보호** 아래에서 **파일** 을 선택합니다.

1. **파일 모니터링 사용** 체크박스를 선택하고 **저장**(아직 선택하지 않은 경우)을 선택합니다.
...
1. **Microsoft Edge** 를 열고 **https://portal.cloudappsecurity.com** 으로 이동한 다음, 클라우드용 Microsoft Defender 앱 포털에 **Joni Sherman** JoniS@WWLxZZZZZZ.onmicrosoft.com 으로 로그인합니다(ZZZZZZ는 랩 호스팅 공급자가 제공한 고유 테넌트 ID임).  Joni Shermans 암호는 랩 호스팅 공급자가 제공합니다.

1. **클라우드용 Microsoft Defender 앱** 포털의 왼쪽 탐색 창에서 **컨트롤** 을 확장하고 **정책** 을 선택합니다.

1. **정책** 페이지에서 **+ 정책 만들기** 를 확장하고 **파일 정책** 을 선택합니다.

1. **파일 정책 만들기** 페이지의 **정책 이름** 필드에 파일의 신용 카드 정보를 입력하고 **설명** 필드에 신용 카드 번호를 공유할 수 없도록 보호하는 정책입니다.를 입력합니다.
...
1. **중요한 정보 유형 선택** 대화 상자에서 **신용 카드 번호** 를 선택하고 오른쪽 위에서 **완료** 를 선택합니다.
...
1. **거버넌스 작업** 섹션에서 **Microsoft SharePoint Online** 을 확장하고 **사용자 격리에 넣기** 를 선택합니다.
...
1. **정책 이름 지정y** 페이지에서 *테넌트 전체 SharePoint 정책* 을 입력하고 **다음** 을 선택합니다.

1. **커넥터 할당** 페이지의 **비업무용|기본값** 탭에서 **SharePoint** 및 **Salesforce** 를 선택한 다음, 페이지 상단에서 **업무용으로 이동** 을 선택합니다.

1. **커넥터 할당** 페이지에서 **업무용** 탭을 선택하여 이제 SharePoint 및 Salesforce가 표시되는지 확인합니다.

1. **다음** 을 두 번 선택합니다.

1. **범위 정의** 페이지에서 **모든 환경 추가** 를 선택하고 **다음** 을 선택합니다.

1. **정책 검토 및 만들기** 페이지에서 정책 설정을 검토하고 **정책 만들기** 를 선택합니다.

이제 사용자가 SharePoint Online 커넥터 및 SalesForce를 제외한 커넥터를 사용하여 흐름을 만들지 못하도록 차단하는 PowerPlatform DLP 정책을 만들었습니다.

# <a name="proceed-to-lab-2---exercise-2"></a>랩 2 - 연습 2로 진행
