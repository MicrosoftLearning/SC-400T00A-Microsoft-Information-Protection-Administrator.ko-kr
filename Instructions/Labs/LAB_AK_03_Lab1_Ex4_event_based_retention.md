...
1. **레이블 설정** 정의 페이지에서 항목 보존을 **영구적으로 또는 특정 기간** 설정에 사용하도록 설정한 다음, 다음**을 선택합니다**.

1. **보존 기간** 정의 페이지에서 5년_ 동안 _** 항목을 보존하도록 **선택합니다.

1. 드롭다운에 **** 따라 보존 기간 시작에서 + 새 이벤트 유형** 만들기를 선택합니다**. 그러면 이벤트 기반 레이블 마법사가 시작됩니다.

1. 오른쪽의 **이름에 이벤트 유형** 플라이아웃을 입력합니다.
...
1. 요약** 페이지를 검토한 **다음 제출을 선택합니다 **.**

1. 이벤트 유형이 **만들어졌**을 때 완료**를 선택합니다**.

1. **새로 만든 **제품 클로저** 이벤트 유형을 선택한 다음 **, 다음**을 선택하여 보존 기간 시작에서 **보존 기간**** 정의로 돌아갑니다.
...
이전 작업에 따라 이제 사용자가 SharePoint 문서의 문서에 적용할 수 있도록 게시된 레이블을 사용할 수 있도록 Project Asset 보존 레이블을 게시합니다.
...
1. **레이블**을 게시할 위치 선택 페이지에서 위치 옵션을 표시할 특정 위치를** 선택하도록 허용을 선택합니다**.

1. 다음 위치만 설정/해제 **되었는지 확인합니다**.

    - **SharePoint 클래식 및 통신 사이트**
    - **OneDrive 계정**

    **Exchange 사서함** 및 **Microsoft 365 그룹 사서함 및 사이트를** 해제**** 해야 합니다.
...
1. 위쪽의 _검색 창에서 브랜드를_ 검색한 다음 검색 결과에서 브랜드** SharePoint 페이지를 선택합니다**.

1. 위쪽 탐색 창에서 문서를** 선택합니다**.

1. 문서** 페이지에서 왼쪽의 **검사 상자를 선택하여 고객 제품 Survey.xlsx**를 선택합니다**. 가로 줄임표를 선택하고 ...****, 단추를 선택한 다음 세부 정보를** 선택합니다**.

1. 오른쪽 메뉴의 속성에서 **레이블** 적용을 선택한 **다음 프로젝트 자산** 레이블을 **선택합니다.**

    >**참고**: 보존 레이블을 게시하는 데 다소 시간이 걸릴 수 있으므로 즉시 사용할 수 있는 옵션이 없을 수 있습니다.

1. 새로 나타난 **자산 ID**에서 NewProductLaunch**를 입력**하고 오른쪽 위 모서리에서 X**를 **선택하여 오른쪽 메뉴를 닫습니다.

문서에 레이블 및 자산 ID를 할당했습니다. 자산 ID NewProductLaunch에 대한 프로젝트 클로저 이벤트가 트리거되면 5년의 보존 기간을 활성화합니다.
...
1. **이벤트 이름 지정** 페이지에서 다음 정보를 설정합니다.
    ...
    - **이름**: 새 제품 출시 종료
    - **설명**: 프로젝트 자산 레이블과 AssetID NewProductLaunch가 있는 자산은 보존 기간에 들어갑니다.

1. **다음**을 선택합니다.

1. 이벤트 설정** 페이지에서 이벤트 유형** 사용을 선택한 **다음, 이벤트 유형** 선택을 선택합니다**.**

1. 이벤트 유형** 선택 페이지에서 프로젝트 클로저**를 선택한 **다음 추가**를 선택합니다**.**

1. **다음**을 선택합니다.

1. **이벤트 설정** 페이지에서 SharePoint 및 OneDrive**의 항목에 대한 자산 ID를 NewProductLaunch**로 **설정합니다**.

1. 이 이벤트가 언제 발생했나요?에 대한 **오늘 날짜를 선택한 다음 다음을 선택합니다****.**

1. 마침** 페이지를 검토한 **다음 제출**을 선택합니다**.

1. **짝수** 만들기 페이지에서 완료**를 선택합니다**.

이벤트를 성공적으로 트리거하고 프로젝트 자산 레이블과 NewProductLaunch의 자산 ID를 사용하여 모든 문서에 대한 보존 기간을 시작했습니다.
...
1. **문서** 페이지에서 왼쪽에 있는 검사 상자를 선택하여 고객 제품 설문 조사.xlsx**를 선택합니다**. 가로 줄임표를 **** 선택합니다.
