...
1. 여전히 클라이언트 1 VM(LON-CL1)에 **lon-cl1\admin** 계정으로 로그인해야 하며, **Joni Sherman** JoniS@WWLxZZZZZZ.onmicrosoft.com 으로 Microsoft 365에 로그인해야 합니다(여기서 ZZZZZZ는 랩 호스팅 공급자가 제공하는 고유한 테넌트 ID임).  Joni의 암호는 랩 호스팅 공급자가 제공합니다.
...
1. **레이블 설정 정의** 페이지에서 **항목 보존 또는 특정 기간** 설정을 사용하도록 설정한 다음, **다음**을 선택합니다.

1. **보존 기간 정의** 페이지에서 _5년_ **동안 항목 보존을** 선택합니다.

1. 보존 **기간 시작** 드롭다운에서 **+ 새 이벤트 유형 만들기를** 선택합니다. 그러면 이벤트 기반 레이블 마법사가 시작됩니다.

1. 오른쪽의 **이벤트 유형** 이름 플라이아웃에서 다음을 입력합니다.
...
1. **이벤트 유형이 만들어졌을 때** **완료**를 선택합니다.

1. 새로 만든 **제품 클로저** 이벤트 유형을 선택하고 **다음****을 선택하여 보존 기간 시작**에서 **보존 기간 정의**로 돌아갑니다.
...
이전 작업에서 게시된 레이블을 사용자가 SharePoint 문서의 문서에 적용할 수 있도록 Project Asset 보존 레이블을 게시합니다.
...
1. **레이블을 게시할 위치 선택** 페이지에서 위치 옵션을 표시할 **특정 위치를 선택하도록 허용**을 선택합니다.

1. 다음 위치만 켜기로 전환되었는지 확인**합니다.**

    - **SharePoint 클래식 및 통신 사이트**
    - **OneDrive 계정**

    **Exchange 사서함** 및 **Microsoft 365 그룹 사서함 & 사이트를** 해제해야 **합니다.**

1. **다음** 단추를 선택합니다.

1. **이름 지정 정책**에서 다음 정보를 입력합니다.
...
1. 위쪽의 검색 창에서 _브랜드를_ 검색한 다음, 검색 결과에서 **Brand** SharePoint 페이지를 선택합니다.

1. 위쪽 탐색 창에서 **문서를** 선택합니다.

1. **문서** 페이지에서 왼쪽의 확인란을 선택하여 **고객 제품 Survey.xlsx** 선택합니다. 가로 줄임표를 선택하고 **...**, 단추를 선택한 다음 세부 **정보를** 선택합니다.

1. 오른쪽 메뉴의 **속성** 에서 **레이블 적용**을 선택한 다음 **프로젝트 자산** 레이블을 선택합니다.
...
1. **이벤트 이름 지정** 페이지에서 다음 정보를 설정합니다.
    ...
    - **이름**: 새 제품 출시 마감
    - **설명**: 프로젝트 자산 레이블과 AssetID NewProductLaunch가 있는 자산은 보존 기간을 입력합니다.
...
1. **이 이벤트가 언제 발생했나요?** 에 대한 오늘 날짜를 선택한 다음, **다음**을 선택합니다.

1. **마침** 페이지를 검토한 다음 **제출**을 선택합니다.

1. **짝수 생성됨** 페이지에서 **완료**를 선택합니다.
...
1. **문서** 페이지에서 왼쪽의 확인란을 선택하여 **고객 제품 Survey.xlsx** 선택합니다. 가로 타 **원...을** 선택합니다.
