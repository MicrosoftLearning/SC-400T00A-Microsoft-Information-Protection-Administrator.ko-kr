...
### <a name="task-1--create-custom-sensitive-information-types"></a>작업 1 - 사용자 지정 중요한 정보 유형 만들기

이 연습에서는 보안 및 준수 센터 PowerShell 모듈을 사용하여 새 사용자 지정 중요한 정보 유형을 만듭니다. 이 유형은 "직원" 및 "ID" 키워드와 비슷한 직원 ID 패턴을 인식합니다.

1. 클라이언트 1 VM(LON-CL1)에 **lon-cl1\admin** 계정으로 로그인되어 있는 상태여야 합니다.

2. **Microsoft Edge** 에서 **https://compliance.microsoft.com** 으로 이동한 다음 Microsoft 365 규정 준수 포털에 JoniS@WWLxZZZZZZ.onmicrosoft.com 으로 로그인합니다(ZZZZZZ는 랩 호스팅 공급자가 제공한 고유 테넌트 ID임).  Joni의 암호는 랩 호스팅 공급자가 제공합니다.
...
7. Contoso 직원 ID 패턴입니다. 를 **설명** 필드에 입력하고 **다음** 을 선택합니다.
...
### <a name="task-2--create-edm-based-classification-information-type"></a>작업 2 - EDM 기반 분류 정보 유형 만들기
...
5. **계정 선택** 페이지가 표시되면 **다른 계정 사용** 을 선택하고 **MOD 관리자**(admin@WWLxZZZZZZ.onmicrosoft.com)로 로그인합니다(여기서 ZZZZZZ는 랩 호스팅 공급자가 제공한 고유 테넌트 ID임).  관리자의 암호는 랩 호스팅 공급자가 제공합니다.
...
21. https://compliance.microsoft.com 에서 Microsoft 365 규정 준수 센터로 이동합니다.
...
39. 위쪽 메뉴에서 더 많은 옵션(...)을 클릭하고 **+ EDM 중요한 정보 유형 만들기** 를 선택하여 **EDM 규칙 패키지** 마법사를 엽니다.  
...
51. **EDM 중요한 정보 유형 이름 지정 및 설명** 페이지에서 이름에 Contoso 직원 EDM을 입력합니다.
...
### <a name="task-3--create-edm-based-classification-data-source"></a>작업 3 - EDM 기반 분류 데이터 원본 만들기

다음으로는 EDM 기반 분류를 중요한 데이터가 포함된 데이터베이스와 연결하기 위해 EDM Upload Agent 도구를 사용하여 중요한 정보 유형에 해당하는 실제 데이터를 해시하여 업로드해야 합니다.

1. 클라이언트 1 VM(LON-CL1)에는 **lon-cl1\admin** 계정으로, Microsoft 365에는 **Joni Sherman** 으로 로그인되어 있는 상태여야 합니다. 

2. **Microsoft Edge** 에서 **https://go.microsoft.com/fwlink/?linkid=2088639** 로 이동하여 EDM 다운로드 에이전트에 액세스합니다.

3. **실행** 을 선택하여 도구를 다운로드한 다음 선택합니다.

4. 설치 마법사가 백그라운드에서 열리면 작업 표시줄에서 설치 관리자를 선택합니다.

5. **Microsoft Exact Data Match Upload Agent 설치 마법사** 에서 **다음** 을 선택합니다.
...
24. **계정 선택** 창이 표시되면 JoniS@WWLxZZZZZZ.onmicrosoft.com으로 로그인합니다(여기서 ZZZZZZ는 랩 호스팅 공급자가 제공한 고유 테넌트 ID임).  Joni의 암호는 랩 호스팅 공급자가 제공합니다.
...
### <a name="task-4--create-keyword-dictionary"></a>작업 4 - 키워드 사전 만들기
...
2. 그리고 **Microsoft Edge** 에는 Office 365 규정 준수 센터 탭이 계속 열려 있어야 합니다. 해당 탭이 열려 있으면 탭을 선택하고 다음 단계를 진행합니다. 닫았다면 새 탭에서 **https://compliance.microsoft.com** 으로 이동합니다.

3. 왼쪽 창에서 **데이터 분류** 를 선택하고 위쪽 창에서 **중요한 정보 유형** 탭을 선택합니다.

4. **+ 중요한 정보 유형 만들기** 를 선택하여 새 중요한 정보 유형 만들기 마법사를 엽니다.

5. **중요한 정보 유형 이름 지정** 페이지의 **이름** 필드에 Contoso 병가 요청 가능 질환 목록을 입력합니다.

6. 직원이 병가 요청 가능한 질병 목록을 입력합니다. 를 **설명** 필드에 입력하고 **다음** 을 선택합니다.
...
### <a name="task-5--work-with-custom-sensitive-information-types"></a>작업 5 - 사용자 지정 중요한 정보 유형 사용
...
8. 그리고 **Microsoft Edge** 에는 Microsoft 365 규정 준수 센터 탭이 계속 열려 있어야 합니다. 해당 탭이 열려 있으면 탭을 선택하고 다음 단계를 진행합니다. 닫았다면 새 탭에서 **https://compliance.microsoft.com** 으로 이동합니다.
...
# <a name="proceed-to-lab-1---exercise-4"></a>랩 1 - 연습 4로 진행 
