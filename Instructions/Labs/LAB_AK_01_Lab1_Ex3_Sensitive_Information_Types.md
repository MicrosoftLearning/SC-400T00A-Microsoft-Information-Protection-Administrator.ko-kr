...
1. 왼쪽 창에서 **데이터 분류** 를 선택합니다.

1. **메시지 데이터 분류** 메시지가 표시되면 **닫기** 를 선택하고 위쪽 창에서 **중요한 정보 유형** 을 선택합니다.  힌트: Purview 포털에 **중요한 정보 유형** 이 표시되지 않으면 이전 랩에서 규정 준수 관리자로 업데이트된 Joni의 권한이 아직 브라우저에서 업데이트되지 않은 것입니다.  로그아웃했다가 JoniS로 다시 로그인해야 할 수 있습니다.
...
1. 위쪽 창에서 **그룹 추가** 를 선택합니다.

1. **그룹 유형 선택** 페이지에서 **보안** 을 선택하고 **다음** 을 선택합니다.

1. **기본 사항 설정** 의 **이름** 필드에 *EDM_DataUploaders* 를 입력합니다.  설명 필드에는 *EDM용 데이터를 업로드하는 사용자* 를 입력합니다.
...
1. **데이터 분류** 를 선택하고 위쪽 창에서 **정확한 데이터 일치** 탭을 선택합니다.

1. **+ EDM 스키마 만들기** 를 선택하여 오른쪽 창을 열고 새 스키마 정의를 만듭니다.

1. **이름** 필드에 *employeedb* 를 입력합니다.

1. **설명** 필드에 *직원 데이터베이스 스키마* 를 입력합니다.
...
1. **EDM 중요한 정보 유형 이름 지정 및 설명** 페이지에서 이름에 Contoso 직원 EDM을 입력합니다.

1. **관리자를 위한 설명** 필드에 *직원 개인 정보용 EDM 기반 중요한 정보 유형입니다.* 를 입력합니다.
...
1. 마우스 오른쪽 단추로 작업 표시줄의 Windows 기호를 선택하고 **Windows PowerShell(관리자)** 을 선택하여 관리자로 PowerShell을 실행합니다.
...
1. 상태가 completed로 변경될 때까지 업로드 진행률을 확인한 후에 다음 PowerShell 명령을 실행합니다.
...
1. **키워드 사전 추가** 페이지에서 *질병 키워드* 이름을 입력합니다. 

1. **키워드** 영역에서 각각 별도의 줄에 다음 키워드를 입력합니다.
...
1. **키워드 목록 추가** 페이지의 **ID** 필드에 *직원 결근* 을 입력합니다. **대/소문자 구분 안 함** 상자에 각각 별도의 줄에 다음 키워드를 입력합니다.
...
1. **Contoso 직원 ID** 를 선택하여 오른쪽 창을 엽니다.

1. 오른쪽 창에서 **테스트** 를 선택합니다.
...
1. **마침** 을 선택하고 **X** 단추를 클릭하여 테스트 페이지를 닫습니다.

1. **데이터 분류** 페이지로 돌아와서 이름이 **Contoso 병가 요청 가능 질환 목록** 인 중요한 정보 유형을 선택합니다.
